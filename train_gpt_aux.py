...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, return_hidden_states: bool = False):
        ...
        if return_hidden_states:
            return ce_loss, logits, hidden_states
        return ce_loss
...
def main() -> None:
    ...
    # fullgraph=True is incompatible with conditional return types in forward()
    # when using auxiliary losses (return_hidden_states creates a graph break)
    ...
    if args.use_aux_losses and args.lambda_unigram > 0:
        ...
        unigram_log_probs = compute_unigram_distribution(
    ...
    if args.use_aux_losses:
        log0(
            ...
            f"decorr={args.lambda_decorr} rank={args.lambda_rank}(every={args.rank_every}) "
            f"unigram={args.lambda_unigram}(decay={args.unigram_decay_frac})"
        )
    log0(
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                if args.use_aux_losses:
                    ...
                    # Option: replace CE with focal loss
                    ...
                    # Representation rank (every N steps to save compute)
