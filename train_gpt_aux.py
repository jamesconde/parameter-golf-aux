...
class Hyperparameters:
    ...
    # torch.compile control (disable for GPUs with limited SM shared memory)
    ...
    focal_label_smoothing = float(os.environ.get("FOCAL_LABEL_SMOOTHING", 0.0))
    lambda_decorr = float(os.environ.get("LAMBDA_DECORR", 0.01))
    decorr_sample_size = int(os.environ.get("DECORR_SAMPLE_SIZE", 64))
    lambda_rank = float(os.environ.get("LAMBDA_RANK", 0.0))
    rank_every = int(os.environ.get("RANK_EVERY", 10))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if flash_attn_3_func is not None:
            ...
            # PyTorch SDPA fallback — expand KV heads for GQA compatibility
            qt = q.transpose(1, 2)
            kt = k.transpose(1, 2)
            vt = v.transpose(1, 2)
...
class GPT(nn.Module):
    ...
    def forward_aux(self, input_ids: Tensor, target_ids: Tensor):
        """Forward pass returning (loss, logits, hidden_states) for aux loss computation."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
...
def main() -> None:
    ...
    if args.use_aux_losses and args.lambda_unigram > 0:
        log0("computing unigram distribution from training data...")
    ...
    while True:
        ...
        aux_loss_total = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                if args.use_aux_losses:
                    ...
                    if args.use_focal_loss:
                        targets = y.reshape(-1)
                        focal = focal_cross_entropy(logits.float(), targets, gamma=args.focal_gamma, label_smoothing=args.focal_label_smoothing)
                        ce_loss = focal
                    if args.lambda_decorr > 0:
                        decorr = inter_layer_decorrelation_loss(hidden_states, sample_size=args.decorr_sample_size)
                        aux_loss = aux_loss + args.lambda_decorr * decorr
                    if args.lambda_rank > 0 and step % args.rank_every == 0:
                        rank_penalty = representation_rank_loss(hidden_states[-1], target_effective_rank_ratio=args.rank_target_ratio)
                        aux_loss = aux_loss + args.lambda_rank * rank_penalty
                    if args.lambda_unigram > 0 and unigram_log_probs is not None:
                        unigram_weight = args.lambda_unigram * max(0.0, 1.0 - step / (args.iterations * args.unigram_decay_frac))
                        if unigram_weight > 0:
        ...
        aux_loss_total /= grad_accum_steps
        ...
        if should_log_train:
            aux_str = f" aux_loss:{aux_loss_total.item():.4f}" if args.use_aux_losses else ""
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f}{aux_str} "
